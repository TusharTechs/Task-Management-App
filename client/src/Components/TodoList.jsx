...
const TodoList = () => {
  ...
  useEffect(() => {
    const getTodos = async () => {
      const response = await fetch("https://task-management-server-1jug.onrender.com/todo/todos", {
  ...
  const toggleTodoState = async (id, currentState) => {
    ...
    const response = await fetch(
      `https://task-management-server-1jug.onrender.com/todo/todos/${id}/${newState}`,
      {
        method: "PATCH",
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      }
  ...
  const deleteTodo = async (id) => {
    try {
      const response = await fetch(`https://task-management-server-1jug.onrender.com/todo/todos/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
